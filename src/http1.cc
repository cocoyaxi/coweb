...
void     RES::add_header(fastring k, fastring v) { header.append(k).append(": ").append(v).append("\r\n"); }
...
Json parse_form_data(fastring& body, bool utf8) {
    ...
    if (name_len == body.npos) { return r; }
    fastring    form_name;
    ...
    for (size_t i = name_len; i < body.size();) {
        ...
        if (body.data()[ptr_name + ptr_name_len + 1] == ';' && body.data()[ptr_name + ptr_name_len + 3] == 'f' &&
            body.data()[ptr_name + ptr_name_len + 4] == 'i')  //; filename
        {
            auto vv = base64_encode(v);
            r.add_member(k.c_str(), vv);
        } else {
            if (utf8) { v = Encode::UTF8ToGBK(v); }
