...
namespace ws {
	...
	void upgrade_to_websocket(Json& req, fastring& buf)
	{
		...
		if (req.has_member("sec-websocket-protocol"))
		{
			...
			header << "Sec-WebSocket-Protocol" << ": " << "protocal_str" << "\r\n";
		}
		header << "\r\n";
		buf = header;
	}
	...
	ws_frame_type parse_payload(const char* buf, size_t size, ws_header& h, fastring& outbuf) {
		...
		if (h.payload_length_ > size)
			return ws_frame_type::WS_INCOMPLETE_FRAME;

		if (h.payload_length_ > outbuf.size()) {
			outbuf.resize((size_t)h.payload_length_);
		}
		...
		if (h.mask_ == 0) {
			...
			// unmask data:
			for (size_t i = 0; i < h.payload_length_; i++) {
				outbuf[i] = inp[i] ^ ((unsigned char*)(&h.mask_))[i % 4];
			}
		}
		...
		if (h.msg_opcode_ == 0x0)
			...
			: ws_frame_type::WS_INCOMPLETE_TEXT_FRAME; // continuation
													   // frame ?
		if (h.msg_opcode_ == 0x1)
			return (h.msg_fin_) ? ws_frame_type::WS_TEXT_FRAME
			: ws_frame_type::WS_INCOMPLETE_TEXT_FRAME;
		if (h.msg_opcode_ == 0x2)
			return (h.msg_fin_) ? ws_frame_type::WS_BINARY_FRAME
			: ws_frame_type::WS_INCOMPLETE_BINARY_FRAME;
		if (h.msg_opcode_ == 0x8)
			return ws_frame_type::WS_CLOSE_FRAME;
		if (h.msg_opcode_ == 0x9)
			return ws_frame_type::WS_PING_FRAME;
		if (h.msg_opcode_ == 0xA)
		...
		while (true)
		{
			...
			if (r > 0)
			{
				...
				if (parse_header(buf.data(), buf.size(), wsh) == 0)
				{
					...
					req.body =(fastring&&) outbuf; req.code_ = code;
