...
std::string Encode::GBKToUTF8(const std::string& str) { return GBKToUTF8(str.c_str()); }
...
std::string    Encode::UTF8ToGBK(const std::string& str) { return UTF8ToGBK(str.c_str()); }
...
std::u16string Encode::UTF32toUTF16(const std::u32string& str) {
    ...
    for (size_t i = 0; i < bytes.size(); i += 2) result.push_back((char16_t)((uint8_t)(bytes[i]) * 256 + (uint8_t)(bytes[i + 1])));
